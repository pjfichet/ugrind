 ...
 *
 * From Heirloom vGrind "vgrindefs.c	1.3 (gritter) 10/22/05"
 */
...
tgetent(char *bp, char *name, char *file)
{
	...
	for (;;) {
		...
		for (;;) {
			...
			if (cp >= bp+BUFSIZ) {
				ugrind_msg = "Ugrind entry too long\n";
				write(2, ugrind_msg, strlen(ugrind_msg));
				break;
			} else
				*cp++ = c;
		}
